...
--*/
{
  ...
  // and I/O creation and synchronization. We only create the device the first
  // time PrepareHardware is called. If the device is restarted by pnp manager
...
--*/
{
  ...
  status = WdfMemoryCreate (&attributes, NonPagedPoolNx, 'MWLU', size, &memory,
  ...
  if (NT_SUCCESS (status)
             ...
             > 0)
    {
      ...
      if (!NT_SUCCESS (status))
        {
          MWLUsb_DbgPrint (1,
                           ("RetrieveDeviceInformation failed %x\n", status));
          return status;
        }
      ...
      //
      // This is written to work with Intel 82930 board, OSRUSBFX2, FX2 MUTT
      // and FX3 MUTT devices. The alternate setting zero of MUTT devices don't
      // have any endpoints. So in the code below, we will walk through the
      // list of alternate settings until we find one that has non-zero
      // endpoints.
      //
      ...
      for (i = 0; i < pDeviceContext->NumberConfiguredPipes; i++)
        {
          ...
          if (!NT_SUCCESS (status))
            {
              MWLUsb_DbgPrint (1,
                               ("InitializePipeContext failed %x\n", status));
              break;
            }
        }
    }
...
--*/
{
  ...
  // Packet Size Constraints: High-speed endpoints are allowed up to 1024-byte
  // data payloads per microframe and allowed up to a maximum of 3 transactions
  // per microframe.
  //
  ...
  // oppurtunities per microframe. 00 - None (1 transaction per microframe) 01
  // - 1 additional (2 per microframe) 10 - 2 additional (3 per microframe) 11
  // - Reserved.
  //
  // Note: MaximumPacketSize of WDF_USB_PIPE_INFORMATION is already adjusted to
  // include additional transactions if it is a high bandwidth pipe.
  //
  ...
  if (pDeviceContext->IsDeviceSuperSpeed == TRUE)
    {
      ...
      if (pDeviceContext->IsStaticStreamsSupported)
        {
          MWLUsb_DbgPrint (
              3, ("Number of Streams supported by the controller:  %d\n",
                  numberOfStreams));
        }
    }
