...
--*/
{
  ...
  status = StringCchPrintfW((PWSTR)wideLinkName, RTL_NUMBER_OF(wideLinkName),
...
--*/
{
  ...
  //
  WDF_USB_DEVICE_SELECT_CONFIG_PARAMS_INIT_MULTIPLE_INTERFACES(&configParams, 0,
                                                               NULL);
  status =
      WdfUsbTargetDeviceSelectConfig(pDeviceContext->WdfUsbTargetDevice,
...
--*/
{
  ...
  if (NT_SUCCESS(status) && WdfUsbTargetDeviceGetNumInterfaces(
                                pDeviceContext->WdfUsbTargetDevice) > 0) {
    ...
    //
    // This is written to work with Intel 82930 board, OSRUSBFX2, FX2 MUTT and
    // FX3 MUTT devices. The alternate setting zero of MUTT devices don't have
    // any endpoints. So in the code below, we will walk through the list of
    // alternate settings until we find one that has non-zero endpoints.
    //
    ...
    for (i = 0; i < numberAlternateSettings && numberConfiguredPipes == 0;
      ...
      WDF_USB_INTERFACE_SELECT_SETTING_PARAMS_INIT_SETTING(&selectSettingParams,
                                                           i);
      ...
      status = WdfUsbInterfaceSelectSetting(
          pDeviceContext->UsbInterface, &pipeAttributes, &selectSettingParams);
...
--*/
{
  ...
  if (pipeInfo.MaximumPacketSize == 0) {
    MWLUsb_DbgPrint(1,
                    ("MaximumPacketSize in the pipeInfo is invalid (zero)\n"));
  ...
  // per microframe.
  //
  // For highspeed isoch endpoints, bits 12-11 of wMaxPacketSize in the endpoint
  // descriptor specify the number of additional transactions oppurtunities per
  // microframe. 00 - None (1 transaction per microframe) 01 - 1 additional (2
  // per microframe) 10 - 2 additional (3 per microframe) 11 - Reserved.
  //
...
RetrieveDeviceInformation(_In_ WDFDEVICE Device) {
  ...
  if (pDeviceContext->IsDeviceSuperSpeed == TRUE) {
    status = WdfUsbTargetDeviceQueryUsbCapability(
        pDeviceContext->WdfUsbTargetDevice, &GUID_USB_CAPABILITY_STATIC_STREAMS,
        sizeof(numberOfStreams), (PUCHAR)&numberOfStreams, NULL);
    if (NT_SUCCESS(status)) {
...
--*/
{
  ...
  *Value = 0;
  ...
  status = WdfDriverOpenParametersRegistryKey(WdfGetDriver(), KEY_READ,
...
VOID MWLUsb_EvtPipeContextCleanup(IN WDFOBJECT WdfObject)
/*++
